...
declare
  t text;
begin
  foreach t in array array['game_state','peserta','pilihan_warna','jurnal','soal']
  loop
...
declare
  t text;
begin
  foreach t in array array['game_state','peserta','pilihan_warna','jurnal']
  loop
    begin
...
begin
  ...
  if v_soal_id is null or v_warna is null then
    return;   -- belum ada putaran aktif, tidak ada yang perlu dibukukan
  end if;

  select debit_benar, kredit_benar, nominal
    into v_debit, v_kredit, v_nominal
  ...
  update jurnal
     set benar = (akun_debit = v_debit and akun_kredit = v_kredit)
  ...
  update jurnal j
     set wajib      = (pw.warna = v_warna),
         diterapkan = (pw.warna = v_warna and j.akun_debit is not null)
    from pilihan_warna pw
